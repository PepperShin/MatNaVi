...
// locationData.js에서 provinces와 cityData를 import 합니다.
...
const TourList = ({ selectedProvince, selectedCity, setSelectedProvince, setSelectedCity }) => {
  ...
  const handleProvinceChange = (e) => {
    ...
    // locationData의 cityData를 사용하여 해당 도의 시/군 데이터가 있을 때 첫 번째 시/군 선택
    setSelectedCity(cityData[newProvince] ? cityData[newProvince][0] : "");
  };
  ...
  // 관광지 정보 가져오기 (useCallback으로 감싸 의존성을 관리)
  ...
  // selectedProvince와 selectedCity 변경 시 fetchTouristData 실행
  useEffect(() => {
  ...
  return (
    <>
        ...
        <Col xs="auto">
          <Form.Select value={selectedCity} onChange={handleCityChange}>
            {cityData[selectedProvince]
              ...
              : <option value="">도 선택 후 시/군 선택</option>}
          </Form.Select>
        </Col>
      ...
      {loading ? <p>로딩 중...</p> : travelList.length === 0 ? <p>결과 없음</p> : currentItems.map((item) => (
        <Link to={`/tripinfo/${item.contentid}`} state={location.state} key={item.contentid} style={{ textDecoration: "none", color: "inherit" }}>
